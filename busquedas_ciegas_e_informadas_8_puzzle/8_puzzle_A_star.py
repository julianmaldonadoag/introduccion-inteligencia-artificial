...
class NodeH:
    def __init__(self, parent, matrix, empty_pos, cost):
        ...
        # Costo f(s) = g(s) + h(s): costo total de transitar desde el estado inicial 
        # hasta el objetivo (pasando por s)
        self.cost = cost 
...
def newNodeA(parent, matrix, empty_pos, new_empty_pos):                
    ...
    # Costo de transitar desde el estado inicial al estado actual.
    cost_g = manhattan(initial_state, new_matrix)
    # Costo de transitar desde el estado actual hasta el estado objetivo
    cost_h = manhattan(new_matrix, goal_state)

    new_node = NodeH(parent, new_matrix, new_empty_pos, cost_g + cost_h)
    return new_node
...
# Solución para el 8-puzzle usando el algoritmo A*.
def A_star(initial_state, empty_pos, goal_state):
    ...
    root = NodeH(None, initial_state, empty_pos, cost_g + cost_h)
    ...
    while frontier:
        # Sale el nodo con el menor costo f(s).
        state = frontier.pop() 
        ...
        for i in range(4): # 4 posibles movimientos.
            ...
            if isValidPos(new_empty_pos[0], new_empty_pos[1]):
                ...
                child = newNodeA(state, state.matrix, state.empty_pos, new_empty_pos)
...
tracemalloc.start()
if (A_star(initial_state, empty_pos, goal_state)):
    end_time = time.time()
    print("Objetivo encontrado aplicando el algoritmo A*")
