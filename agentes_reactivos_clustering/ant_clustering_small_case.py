...
Description: 
    Clusterización de objetos basado en el comportamiento de las hormigas. Para este caso
    de prueba la cantidad de celdas por filas y columnas es pequeña lo que no permite utilizar
    esos valores como el ancho y alto en pixeles de la pantalla porque seria diminuta, 
    de igual manera ya no es conveniente imprimir un pixel por celda sino un conjunto de pixeles.
    Es por eso que cada vez que se dibuja una celda que tiene un objeto para este caso
    se dibuja un cuadrado y ya no un solo pixel (como en el caso de prueba original).
'''
...
def main():
    width_screen, height_screen = 400, 400

    # Parametros.
    num_cols, num_rows = 50, 50 # Numero de columnas y filas.
    num_objects = 500
    num_agents = 10
    k1 = 0.1
    k2 = 0.3
    ...
    pygame.init()
    pygame.display.set_caption('Ant clustering small case')
